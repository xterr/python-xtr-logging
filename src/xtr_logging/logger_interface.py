...
class LoggerInterface(Protocol):
    ...
    a channel of a :class:`~xtr_logging.logger.Logger`, a
    :class:`~xtr_logging.null_logger.NullLogger`, an adapter over a standard
    library logger — can be swapped without touching a call site.

    The rules:

    - ``message`` may hold ``{key}`` placeholders naming entries of
      ``context``. The logger does not substitute them; a
      :class:`~xtr_logging.processor.placeholder_processor.PlaceholderProcessor`
      does, so a handler may still see the template and the values apart.
